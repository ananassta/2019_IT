...
def trapezoidal(f, a, b, n,g,y):
    res1=0
    res2=0
    for i in range(a,y-1):
        i_first=i
        i_second=i+1
        h = float(g[i_second] - g[i_first])/2
        result = 0.5*(f[i_first] + f[i_second])*h
        res1  =res1 + result
    for i in (y,b-1):
        i_first=i
        i_second=i+1
        h = float(g[i_first] - g[i_second])/2
        result = 0.5*(f[i_first] + f[i_second])*h
        res2  =res2 + result
    if res1<0 :
        res1=res1*(-1)
...
while S_1 != S_2 :
    ...
    #print (f"{y0}")
    if (y0>70):
        print ("все пошло по гг")
        break
    for i in range(0,i_end):
        if (x[i]==x[y0]or(x[i+1]>x[y0] and x[i]<x[y0])):
            i_1 = i
    for i in range(i_start, len(x)-1):
        if (x[i+1]==x[y0]or (x[i+1]>x[y0] and x[i]<x[y0])):
            i_2=i+1
    #теперь находим площади этих частей
    #для этого найдем сколько отрезков входит в каждую часть
    n_1=y0-i_1
    n_2=i_2 - y0
    S_1 = float( trapezoidal(u,i_1,y0,n_1,x,i_end))
    #S_2 = float(trapezoidal(u,i_2,y0,n_2,x))
    S_2 = float(trapezoidal2(u,y0,i_2,n_2,x,i_start))
    S_1=round(S_1,2)
    S_2=round(S_2,2)
    #S_2=S_2*(-1)
    #print (f"{S_1}")
